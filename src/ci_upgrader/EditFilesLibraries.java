...
public class EditFilesLibraries {

    //|-------->  |RUN METHOD|  <-------|
    public void run(Path path_ci3, Path path_ci4) throws IOException {
        System.out.println("\n" + "###  |Libraries|  ###");

//----- ###  |Edit Libraries Subfoldernames|  ###

        File tar = new File (path_ci4 + "\\app\\Libraries");

        editSubfolderNames(tar);
        outputFoldernames(tar);

//----- ###  |Edit Libraries Files|  ###

        ArrayList<File> librarieslist = new ArrayList<>();
        getLibrariesList(tar, librarieslist);

        for (int i = 0; i < librarieslist.size(); i++){
            editFiles(librarieslist.get(i));
        }
    }

    //|-------->  |METHOD TO RENAME LIBRARIES SUBFOLDERS (JUST FIRST LETTER UPPERCASE)|  <-------|
    ...
    //|-------->  |METHOD TO GET ALL LIBRARIES FILES|  <-------|
    public void getLibrariesList (File tar, ArrayList<File> librarieslist){

        for (File file : tar.listFiles()){

            if (file.isDirectory()){
                getLibrariesList(file, librarieslist);}
    ...
    //|-------->  |METHODS TO EDIT CONTENT OF LIBRARIES FILES|  <-------|
    public void editFiles (File tar)throws IOException{

        if (filesedit_count != 0){libraries_log = libraries_log + "-------------------------------------------------------------------------------\n\n";}

        editFilesHeader(tar);

        outputFiles(tar);
        libraries_log = libraries_log + buildUpgradeLogString(tar.getName());
        filesedit_count++;
    }
    ...
    private void editFilesHeader(File tar) throws IOException {
        ...
        while ((line = brtar.readLine()) != null) {               //Read target file line by line
            ...
            if (line.contains("<?php") && phptag == 0){
                ...
                countfiles++;
                //UpgradeLog
                edit_upgrade_log = edit_upgrade_log + "   - [Line " + (countline + 1) + "]: " + namespace + "   |added|\n";
                countline++;
                edit_numb++;
            }
    ...
    public String getNamespace (File tar){
        ...
        index = path.indexOf("app\\Libraries");
    ...
    public void outputFoldernames (File tar){
        if (countfoldernames == 0){System.out.println("No subfolders found/edited in app/Libraries/");}
        else if (countfoldernames == 1){System.out.println(countfoldernames + " subfoldername edited in app/Libraries/ : ");}
        else {System.out.println(countfoldernames + " subfoldernames edited in app/Libraries/ : ");}
    ...
    protected static String libraries_log = "";
    ...
    public String getUpgradeLog(Path path_ci3, Path path_ci4) throws IOException{
        upgradelog = "################################  |" + "LIBRARIES" + "| ################################\n";

        upgradelog = upgradelog + "[CI3] '" + path_ci3 + "\\application\\language'\n";
        upgradelog = upgradelog + "[CI4] '" + path_ci4 + "\\app\\Language'\n\n";
        ...
        if (files_numb == 0){
            ...
            upgradelog = upgradelog + libraries_log;
        }
